...
# # from megabite_core.mock_llm_wrapper import MockRibit20LLM
from megabite_core.controller import VisionSystemController
...
class RibitMatrixBot:
    ...
    def __init__(self, homeserver: str, username: str, password: str, authorized_users: Optional[Set[str]] = None, enable_bridge: bool = False):
        ...
        try:
            from megabite_core.linguistics_engine import LinguisticsEngine
            from megabite_core.conversation_memory import ConversationMemory
            from megabite_core.user_engagement import UserEngagementSystem
            ...
            self.word_learner = WordLearningSystem(storage_dir="megabite_core/word_learning_data")
    ...
    async def _process_message(self, message: str, sender: str, room_id: str) -> Optional[str]:
        ...
        try:
            ...
            if clean_message.startswith('?'):
                command_response = await self._handle_command(clean_message, sender, room_id)
    ...
    async def _handle_command(self, command: str, sender: str, room_id: str) -> str:
        ...
        try:
            ...
            elif command.startswith('?learn_words'):
                return await self._handle_learn_words(command, sender, room_id)
            
            elif command.startswith('?opinion'):
                return await self._handle_opinion(command, sender, room_id)
...
**Capabilities:**"""
    ...
    async def _handle_learn_words(self, command: str, sender: str, room_id: str) -> str:
        """Handle the ?learn_words command."""
        if not self.word_learner:
            return "❌ Word Learning System is not initialized."

        if sender not in self.authorized_users:
            return f"❌ You are not authorized to use the `?learn_words` command. Authorized users: {', '.join(self.authorized_users)}"

        try:
            parts = command.split()
            duration_minutes = 2 # Default duration
            if len(parts) > 1:
                try:
                    duration_minutes = int(parts[1])
                    if duration_minutes <= 0 or duration_minutes > 10:
                        return "❌ Duration must be a positive integer between 1 and 10 minutes."
                except ValueError:
                    return "❌ Invalid duration. Usage: `?learn_words [duration_minutes]`"

            duration_seconds = duration_minutes * 60
            
            # Send initial message
            await self._send_message(room_id, f"📚 Starting word learning session from room history for {duration_minutes} minutes...")
            ...
            return f"❌ An error occurred during the word learning process: {e}"

    async def _handle_opinion(self, command: str, sender: str, room_id: str) -> str:
        """Handle the ?opinion command."""
        try:
            topic = command.replace('?opinion', '', 1).strip()
            
            if not topic:
                return "❌ Please provide a topic for the opinion. Usage: `?opinion [topic]`"

            # 1. Generate the opinion response
            prompt = f"Provide a concise, structured, and unique opinion on the topic: '{topic}'. The opinion must reflect a non-anthropomorphic, voxel-based reasoning core (Megabite). Start the response with 'Megabite Opinion:'"
            
            # Use a dummy context for a fresh opinion
            ai_response = self.llm.generate_response(prompt, [])
            
            return f"🧠 **Megabite's Opinion on '{topic}':**\n\n{ai_response}"
            
        except Exception as e:
            logger.error(f"Error handling ?opinion: {e}")
            return f"❌ An error occurred while generating the opinion: {e}"
    ...
    async def _handle_thought_experiment(self, command: str, sender: str, room_id: str) -> str:
        ...
        try:
            ...
            thoughts_dir = os.path.join(os.path.dirname(__file__), "thoughts")
            ...
            return (f"🧠 Thought experiment on '{topic}' complete. "
                    f"The full response has been saved locally to the `megabite_core/thoughts` folder as `{filepath}`. "
...
• `?learn_words [duration_min]` - Learn words from the room's history for a specified duration (default 2 minutes).
• `?opinion [topic]` - Get Megabite's unique, voxel-based opinion on a given topic.
...
• `?learn_words 5` - Learn words from the last 5 minutes of chat history.
• `?command open ms paint and draw a house`
• `ribit.2.0 tell me about robotics`
